...
bool peer_identity_process(PeerData* v, const char* addr, bool is_banned, uint64_t timeout, bool do_timeout)
{
	...
	{
        if (v->op < 2 && (cJSON_HasObjectItem(ip_addr_list, addr) || cJSON_HasObjectItem(ip_addr_list, v->udid.value)) && g_config.ip_validation)
		{
...
bool server_cmd_handle(Server* server, unsigned long hash, PeerData* v, String* msg)
{
	...
	switch(hash)
	{
        ...
        {
            uint8_t page = 1;
            sscanf(msg->value, ":status %u", &page);
            char format[100];
            snprintf(format, 128, "----" CLRCODE_YLW "status " CLRCODE_GRN "page %u" CLRCODE_RST "----", page);
            ...
            switch (page) {
                case 1:
                    ...
                    RAssert(server_send_msg(v->server, v->peer, format));
                        break;
                case 2:
                    snprintf(format, 100, "tails shots and hits: \\%u~:@%u~", g_status.tails_shots, g_status.tails_hits);
                    RAssert(server_send_msg(v->server, v->peer, format));
                    snprintf(format, 100, "cream rings spawned: @%u~", g_status.cream_rings_spawned);
                    RAssert(server_send_msg(v->server, v->peer, format));
                    snprintf(format, 100, "eggman mines planted: @%u~",  g_status.eggman_mines_placed);
                    RAssert(server_send_msg(v->server, v->peer, format));
                    snprintf(format, 100, "total stuns: %u", g_status.total_stuns);
                    RAssert(server_send_msg(v->server, v->peer, format));
                    break;
                case 3:
                    snprintf(format, 100, "exeller clone \\placed: %u~, @activated: %u~", g_status.exeller_clones_placed, g_status.exeller_clones_activated);
                    RAssert(server_send_msg(v->server, v->peer, format));
                    snprintf(format, 100, "exetior bring spawn: %u~", g_status.exetior_bring_spawned);
                    RAssert(server_send_msg(v->server, v->peer, format));
                    snprintf(format, 100, "damage dealt: %u", g_status.damage_taken);
                    RAssert(server_send_msg(v->server, v->peer, format));
                    break;
                case 4:
                    snprintf(format, 100, "%u left during games overall", g_status.timeouts);
                    RAssert(server_send_msg(v->server, v->peer, format));
                    break;
                default:
		...
		case CMD_HELP:
		{
            ...
            switch(page){
                case 1:
                    ...
                    break;
                case 2:
                    snprintf(lobby_msg, 128, CLRCODE_GRA CLRCODE_GRA ".map" CLRCODE_RST " choose map (1-%d)", MAP_COUNT);
                    RAssert(server_send_msg(v->server, v->peer, lobby_msg));
			...
			case CLIENT_LOBBY_CHOOSEOP:
			{
				...
				for (size_t i = 0; i < v->server->peers.capacity; i++)
				{
					...
					if (peer->id == pid)
					{
                        ...
                        RAssert(op_add(peer->nickname.value, peer->ip.value, g_config.op_default_level, "Default Note..."));
