...
bool rmz_tick(Server* server)
{
	...
	{
        if (server->game.time_sec <= g_config.ring_appearance_timer && server->game.bring_state < BS_DEACTIVATED)
			game_bigring(server, BS_DEACTIVATED);

        if (server->game.time_sec <= g_config.escape_time && server->game.bring_state < BS_ACTIVATED)
		{
            if((7 - game_find(server, NULL, "shard", 7) >= 6) || g_config.rmz_easy_mode)
