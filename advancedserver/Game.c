...
bool game_demonize(Server* server, PeerData* data)
{
    ...
    if (players / 2 > demonized || g_config.demonic_madness)
	{
	...
	switch (type)
	{
		...
		case CLIENT_PLAYER_DEATH_STATE:
		{
			...
			if (dead)
			{
                ...
                if (v->plr.flags & PLAYER_REVIVED || (v->server->game.time_sec < g_config.sudden_death_timer && !g_config.disable_timer) || (v->server->game.time_sec > g_config.sudden_death_timer && g_config.disable_timer))
				{
...
bool game_player_tick(Server* server)
{
    ...
    ((server->game.time_sec <= g_config.sudden_death_timer && !g_config.disable_timer) ||
    (server->game.time_sec >= g_config.sudden_death_timer && g_config.disable_timer)))
	{
