		...
		case CLIENT_CHAT_MESSAGE:
		{
			...
			switch (hash)
			{
                ...
                case CMD_CHANCE:
                {
                    ...
                    uint8_t chance;
                    if (sscanf(msg.value, ":chance %u", &chance) > 100)
                    {
                        RAssert(server_send_msg(v->server, v->peer, CLRCODE_RED "example:~ :chance 0-100"));
                        break;
                    }
                    ...
                    RAssert(server_send_msg(v->server, v->peer, CLRCODE_GRN "code activated." CLRCODE_RST));
                    break;
                }
