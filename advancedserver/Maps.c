...
{
    if (!g_config.gameplay.banana.disable_timer && !g_config.gameplay.gmcycle.overhell) {
        if (server->game.time_sec <= g_config.gameplay.ring_appearance_timer && server->game.bring_state < BS_DEACTIVATED)
...
{
    server->game.time_sec = g_config.gameplay.banana.disable_timer ? 0 : (uint16_t)(seconds + (((server_ingame(server) - 1) * mul)));
	return true;
}
