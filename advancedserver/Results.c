...
bool results_send(Server* server, PeerData* v, PeerData* data, bool has_quit)
{
	...
	if (g_config.results_misc.pride)
	{
    ...
    PacketWrite(&pack, packet_writestr, g_config.gameplay.anonymous_mode ? string_new("anonymous") : nickname);
...
bool results_init(Server* server)
{
    ...
    server->results.countdown = g_config.results_misc.results_timer * TICKSPERSEC;
...
bool results_state_handle(PeerData* v, Packet* packet)
{
	...
	switch (type) 
	{
		...
		{
			PeerData** sort = malloc(sizeof(PeerData*) * g_config.pairing.maximum_players_per_lobby);
			if (!sort) {
			...
			int len = 0;
            memset(sort, 0, sizeof(PeerData*) * g_config.pairing.maximum_players_per_lobby);
