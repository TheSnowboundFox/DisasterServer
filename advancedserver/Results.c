...
bool results_send(Server* server, PeerData* v, PeerData* data, bool has_quit)
{
    ...
    PacketWrite(&pack, packet_writestr, g_config.lobby_misc.anonymous_mode ? string_new("anonymous") : nickname);
...
bool results_init(Server* server)
{
    ...
    server->results.countdown = g_config.results_misc.timer * TICKSPERSEC;
