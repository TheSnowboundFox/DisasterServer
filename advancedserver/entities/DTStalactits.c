...
bool dtst_tick(Server* server, Entity* entity)
{
	...
	if (titi->state)
	{
		titi->vel += g_config.gameplay.entities_misc.map_specific.dark_tower.stalactites.acceleration * (float)server->delta;
...
bool dtst_activate(Server* server, DTStalactits* tits)
{
	...
	tits->show = false;
	tits->state = false;
    tits->timer = (g_config.gameplay.entities_misc.map_specific.dark_tower.stalactites.timer
		+ (!g_config.gameplay.entities_misc.map_specific.dark_tower.stalactites.timer_offset
		? 0 : (rand() % g_config.gameplay.entities_misc.map_specific.dark_tower.stalactites.timer_offset))) * TICKSPERSEC;
	tits->pos.y = tits->sy;
	tits->vel = 0;
