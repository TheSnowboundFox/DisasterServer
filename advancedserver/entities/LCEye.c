...
bool lceye_tick(Server* server, Entity* entity)
{
	...
	if (eye->timer >= TICKSPERSEC)
	{
		...
		{
            eye->charge -= g_config.gameplay.entities_misc.map_specific.limb_city.eye.use_cost;
			if (eye->charge < 20)
			{
                eye->cooldown = g_config.gameplay.entities_misc.map_specific.limb_city.eye.recharge_timer * TICKSPERSEC;
		...
		{
            eye->charge += g_config.gameplay.entities_misc.map_specific.limb_city.eye.recharge_strength;
			RAssert(lceye_update(server, eye));
		}
