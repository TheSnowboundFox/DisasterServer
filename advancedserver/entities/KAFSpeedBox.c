...
bool kafbox_activate(Server* server, KafBox* box, uint16_t pid, uint8_t is_proj)
{
	...
	box->activated = true;
    box->timer = (g_config.gameplay.entities_misc.map_specific.kind_and_fair.speedbox_timer
		+ (!g_config.gameplay.entities_misc.map_specific.kind_and_fair.speedbox_timer_offset
			? 0 : (rand() % g_config.gameplay.entities_misc.map_specific.kind_and_fair.speedbox_timer_offset))) * TICKSPERSEC;
