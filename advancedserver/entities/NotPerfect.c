...
bool npctrl_tick(Server* server, Entity* entity)
{
	...
	switch (ctrl->state)
	{
		case NPC_NONE:
		{
            const int intr1 = server->game.time_sec < g_config.ring_appearance_timer  && !g_config.disable_timer ? g_config.switch_warning_timer_chase : g_config.switch_warning_timer;
		...
		case NPC_PREPARE:
		{
            const int intr2 = server->game.time_sec < g_config.ring_appearance_timer && !g_config.disable_timer ? g_config.switch_timer_chase : g_config.switch_timer;
