...
bool mapvote_init(Server* server)
{
    ...
    server->lobby.countdown_sec = g_config.map_selection.mapvote_timer;
	server->lobby.countdown = TICKSPERSEC;
	...
	MutexLock(g_config.map_list_lock);
	{
        ...
        for (int8_t i = 0; i < MAP_COUNT; i++)
        {
            if (g_config.map_selection.map_list[i] && (!g_config.map_selection.exclude_last_map || i != server->last_map))
                allowed[allowed_count++] = i;
        }

        if (!g_config.map_selection.use_mapvote)
        {
...
{
    if(server_ingame(v->server) <= 1 - g_config.gameplay.banana.singleplayer)
