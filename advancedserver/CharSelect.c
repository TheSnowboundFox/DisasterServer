...
bool charselect_check_state(Server* server)
{
    ...
    if (should){
        if(g_config.charselect_mod_unlocked && !g_config.anonymous_mode)
            for (size_t i = 0; i < server->peers.capacity; i++)
            {
                PeerData* peer = (PeerData*)server->peers.ptr[i];
                if (!peer)
                ...
                // We already have survivor characters assigned, let's send the packets
                Packet pack;

                PacketCreate(&pack, SERVER_LOBBY_CHARACTER_RESPONSE);
                PacketWrite(&pack, packet_write8, peer->surv_char + 1);  // ID of the character (1-based index)
                PacketWrite(&pack, packet_write8, true);                 // Availability (always true now)
                RAssert(packet_send(peer->peer, &pack, true));           // Send to each peer

                PacketCreate(&pack, SERVER_LOBBY_CHARACTER_CHANGE);
                PacketWrite(&pack, packet_write16, peer->id);            // Peer ID
                PacketWrite(&pack, packet_write8, peer->surv_char + 1);  // Character ID
                server_broadcast(server, &pack, true);                   // Broadcast to everyone
            }
...
bool charselect_state_handle(PeerData* v, Packet* packet)
{
	...
	{			
		case CLIENT_REQUEST_EXECHARACTER:
		{
            ...
            if(g_config.allow_foreign_characters){
		...
		case CLIENT_REQUEST_CHARACTER:
		{
            ...
            if (avail && !g_config.charselect_mod_unlocked)
            ...
            if(!g_config.anonymous_mode && !g_config.charselect_mod_unlocked){
                PacketCreate(&pack, SERVER_LOBBY_CHARACTER_CHANGE);
                PacketWrite(&pack, packet_write16, v->id);
                PacketWrite(&pack, packet_write8, id+1);
                server_broadcast(v->server, &pack, true);
...
bool charselect_init(int8_t map, Server* server)
{
    ...
    server->lobby.countdown_sec = g_config.charselect_timer;
	server->lobby.countdown = TICKSPERSEC;
...
bool charselect_state_left(PeerData* v)
{
    ...
    if (server_ingame(v->server) <= (1 - g_config.singleplayer) || v->id == v->server->lobby.exe)
